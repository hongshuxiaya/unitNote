...
def get_index_note(url=None, headers=None):
    startindex = str(0)
    rows = str(99)
    if url is None:
        url = host + data_config()['getindex_note']['path'].replace('{userid}', userid)
        url = url.replace('{startindex}', startindex).replace('{rows}', rows)
    if headers is None:
        headers = header
    get_res = get(url, headers=headers)
...
def get_calen_note(data=None, headers=None):
